...
//    uint nny = Pa.Ny + 2 * Pa.PMLy;
    ...
    h_U[iz * block_x * block_y + iy * block_x + ix] += Wave * (Pa.dt * Pa.dt * h_Vp[iz * temph_Vp.length_x * temph_Vp.length_y + iy * temph_Vp.length_x + ix]);
}
              ...
              const Partition& pt, int it)
{
    ...
    // 空间8阶精度的交错网格有限差分
    for (uint iz = 0 + gap_min_z; iz < block_z + temph_U.topborder + gap_max_z; iz++)
    {
        for (uint iy = 0 + gap_min_y; iy < block_y + temph_U.backborder + gap_max_y; iy++)
        {
            for (uint ix = 0 + gap_min_x; ix < block_x + temph_U.leftborder + gap_max_x; ix++)
            {
...
//    uint nnz = Pa.Nz + 2 * Pa.PMLz;
    ...
    // 空间8阶精度的交错网格有限差分
    for (uint iz = 0 + gap_min_z; iz < block_z + h_VW.topborder + gap_max_z; iz++)
    {
        for (uint iy = 0 + gap_min_y; iy < block_y + h_VW.backborder + gap_max_y; iy++)
        {
            for (uint ix = 0 + gap_min_x; ix < block_x + h_VW.leftborder + gap_max_x; ix++)
            {
               ...
               const Partition& pt, const MPI_Comm &mycomm)
{
    ...
    for(int i = 0; i < block_z; ++i)
    {
        ...
        {
            memcpy(plan->h_Vp + (i + temph_Vp.topborder) * temph_Vp.length_y * temph_Vp.length_x + (j + temph_Vp.backborder) * temph_Vp.length_x + temph_Vp.leftborder,	ip->TrueVp + i * j * block_x,	block_x * sizeof(float));
        }
    }
    ...
    for (uint is = 0; is < ip->ShotN; is++)
    {
        ...
        for (uint it = 0; it < Pa.Nt; it++)
        {
            for(int i = 0; i < block_z; ++i)
            {
                ...
                {
                    // 波场时刻转换
                    memcpy(plan->h_U_past + i * block_y * block_x + j * block_x, plan->h_U_now + (i + temph_U.topborder) * temph_U.length_y * temph_U.length_x + (j + temph_U.backborder) * temph_U.length_x, sizeof(float) * block_x);
                    memcpy(plan->h_U_now + (i + temph_U.topborder) * temph_U.length_y * temph_U.length_x + (j + temph_U.backborder) * temph_U.length_x, plan->h_U_next + i * block_y * block_x + j * block_x, sizeof(float) * block_x);
                }
            }
             ...
             const MPI_Comm& mycomm)
{
    ...
    // 把梯度变量空间初始化
    memset((void *)plan->h_Grad,		0,	sizeof(float) * Pa.Nz * Pa.Ny * Pa.Nx);

    // 给速度赋值
    memset((void *)plan->h_Vp,			0,	sizeof(float) * nnz * nny * nnx);
    memcpy(plan->h_Vp,	ip->CurrVp,	 nnz * nny * nnx * sizeof(float));

    // 对每一炮进行迭代
    for (uint is = 0; is < ip->ShotN; is++)
    {
        memset((void *)plan->h_PHIx_U_x,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIy_U_y,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIz_U_z,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIx_V_x,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIy_K_y,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIz_W_z,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_past,		0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_now,		0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_next,		0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_V,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_K,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_W,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIx_U_x_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIy_U_y_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIz_U_z_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIx_V_x_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIy_K_y_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_PHIz_W_z_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_past_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_now_r,		0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_U_next_r,	0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_V_r,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_K_r,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_W_r,			0,	sizeof(float) * nnz * nny * nnx);
        memset((void *)plan->h_re,			0,	sizeof(RL) * ip->St[0].rn);
        ...
        memset((void *)plan->h_U_freq,		0,	sizeof(Complex) * Pa.Nz * Pa.Ny * Pa.Nx * ip->FreqN);
        memset((void *)plan->h_U_freq_r,	0,	sizeof(Complex) * Pa.Nz * Pa.Ny * Pa.Nx * ip->FreqN);
        ...
        {
            // 波场时刻转换
            memcpy(plan->h_U_past, plan->h_U_now, sizeof(float) * nnz * nny * nnx);
            memcpy(plan->h_U_now, plan->h_U_next, sizeof(float) * nnz * nny * nnx);
            ...
            for (uint nf = 0; nf < ip->FreqN; nf++)
            {
                StepCalWFFreq(Pa, plan->h_U_now, StartFreq + (float)nf * InterFreq,
                    it, plan->h_U_freq + nf * Pa.Nz * Pa.Ny * Pa.Nx, pt);
            }
        ...
        // 计算残差波场
        StepResidualConj(Pa, plan, ip->St[is].rn);
        ...
        {
            // 波场时刻转换
            memcpy(plan->h_U_past_r, plan->h_U_now_r, sizeof(float) * nnz * nny * nnx);
            memcpy(plan->h_U_now_r, plan->h_U_next_r, sizeof(float) * nnz * nny * nnx);

            // 一步计算频率域波场
            for (uint nf = 0; nf < ip->FreqN; nf++)
            {
                StepCalWFFreq(Pa, plan->h_U_now_r, StartFreq + (float)nf * InterFreq,
                    it, plan->h_U_freq_r + nf * Pa.Nz * Pa.Ny * Pa.Nx, pt);
            }
            ...
            AddResidual(Pa, plan->h_ResWF, plan->h_U_next_r,
                plan->h_re, it, ip->St[is].rn, plan->h_Vp, pt, it);
        ...
        for (uint nf = 0; nf < ip->FreqN; nf++)
        {
            ...
            // 第1步，乘以-w^2
            ComplexMulConst(plan->h_U_freq_r + nf * Pa.Nz * Pa.Ny * Pa.Nx,
                plan->h_U_freq_r + nf * Pa.Nz * Pa.Ny * Pa.Nx,
                a,
                Pa.Nz * Pa.Ny * Pa.Nx);

            // 第2步，计算负梯度方向
            StepCalGradFreq(Pa, plan->h_Grad,
                plan->h_U_freq + nf * Pa.Nz * Pa.Ny * Pa.Nx,
                plan->h_U_freq_r + nf * Pa.Nz * Pa.Ny * Pa.Nx, pt);
        }
    ...
    // 输出梯度
    memcpy(ip->GradVp, plan->h_Grad, Pa.Nz * Pa.Ny * Pa.Nx * sizeof(float));
                   ...
                   const MPI_Comm& mycomm)
{
    ...
    // 将梯度进行归一化
    float MV = MaxValue(ip->GradVp, Pa.Nz * Pa.Ny * Pa.Nx);
    if (MV > 1.0e-10f)
    {
        for (uint m = 0; m < Pa.Nz * Pa.Ny * Pa.Nx; m++)
        {
            ip->GradVp[m] /= MV;
        }
    }
    ...
    // 生成试探速度模型
    memset((void *)plan->h_Grad,	0,				sizeof(float) * Pa.Nz * Pa.Ny * Pa.Nx);
    memcpy(plan->h_Grad,			ip->GradVp,		sizeof(float) * Pa.Nz * Pa.Ny * Pa.Nx);
    memcpy(plan->h_Vp,				ip->CurrVp,		sizeof(float) * nnx * nny * nnz);
    ...
    for (uint is = 0; is < ip->ShotN; is++)
    {
        ...
        memcpy(plan->h_CurrWF,
            sgs_c + is * (Pa.Nt * ip->St[is].rn),
            Pa.Nt * ip->St[is].rn * sizeof(float));

        // 计算上述两个炮集的残差
        StepResidual(Pa, plan->h_TrailWF, plan->h_CurrWF,
            plan->h_TrailWF, ip->St[is].rn);
        ...
        {
            // 初始化变量
            memset((void *)plan->h_ResTrail_freq,	0,	sizeof(Complex) * ip->St[0].rn);
            memset((void *)plan->h_ResCurr_freq,	0,	sizeof(Complex) * ip->St[0].rn);

            // 计算频率域炮集
            CalSGFreq(Pa, Pa.Nt, ip->St[0].rn,
            ...
            // 计算步长公式中的分子
            memset((void *)plan->h_Temp,	0,	sizeof(float) * ip->St[0].rn);
            ...
            for (uint ir = 0; ir < ip->St[0].rn; ir++)
            {
                ...
                const MPI_Comm& mycomm)
{
    ...
    memcpy(ip->CurrVp, plan->h_Vp, sizeof(float) * nnz * nny * nnx);
